...
function DichVu() {
  ...
  const fetchDichVus = async () => {
    setLoading(true);
    try {
      const response = await apiFetch('http://localhost:5189/api/DichVu?pageNumber=1&pageSize=100');
      if (response.ok) {
  ...
  const handleAddService = async (values) => {
    try {
      const response = await apiFetch('http://localhost:5189/api/DichVu', {
        method: 'POST',
  ...
  const handleSearch = async (values) => {
    try {
      const response = await apiFetch(`http://localhost:5189/api/DichVu/search?tenDichVu=${values.tenDichVu}`);
      if (response.ok) {
        const data = await response.json();
